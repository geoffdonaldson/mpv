...
static void add_stream_headers(struct priv *p)
{
    ...
    for (int n = p->wrapper->num_streams; n < p->demuxer->num_streams; n++) {
        ...
        // Future directions: remove all "special" fields from the data
        // structure, and allow sharing the headers directly.
        struct sh_stream orig = *dst;
        *dst = *src;
        dst->demuxer = orig.demuxer;
        dst->audio = orig.audio;
        if (dst->audio) {
            *dst->audio = *src->audio;
            dst->audio->gsh = dst;
        }
        dst->video = orig.video;
        if (dst->video) {
            *dst->video = *src->video;
            dst->video->gsh = dst;
        }
        ...
        // The packet queue was also exchanged.
        assert(packet_queue_is_empty(orig.pq));
        talloc_free(orig.pq);
    }
